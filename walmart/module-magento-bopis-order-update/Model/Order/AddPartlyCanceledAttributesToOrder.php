 ...
 * - Adds order history comment with Partly Canceled notification information.
 */
class AddPartlyCanceledAttributesToOrder
{
    /**
    ...
    public function execute(OrderInterface $order, string $state, string $status): void
    {
        ...
        // Add order history item with Partly Canceled information.
        ...
        $history = $order->addCommentToStatusHistory(
            __('Order notified as partly canceled at: %1', $time),
            $order->getStatus(),
            true
        );
